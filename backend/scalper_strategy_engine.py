...
from performance_tracker import get_dynamic_risk, print_performance_summary, log_trade
...
def determine_lot_size(symbol, sl_price, entry_price, fixed_lot, strategy_mode):
    ...
    risk_percent *= dynamic_multiplier
...
def process_symbol_cycle(symbol, strategy_mode="zone_reversal_phase4", fixed_lot=None):
    try:
        ...
        if mt5.symbol_info_tick(symbol) is None:
            return
        tick = mt5.symbol_info_tick(symbol)
        ...
        atr = AverageTrueRange(m5_df['high'], m5_df['low'], m5_df['close']).average_true_range().iloc[-1]
        m5_context = {'trend': calculate_trend(m5_df)}

        for z in demand_zones:
            if abs(tick.bid - z["price"]) <= max(atr, 50 * get_symbol_spec(symbol).point):
        ...
        signals, _ = run_trade_decision_engine(
            symbol=symbol,
            point=get_symbol_spec(symbol).point,
            ...
            SL_BUFFER=0,
            TP_RATIO=TP_RATIO,
            CHECK_RANGE=max(atr, 50 * get_symbol_spec(symbol).point),
            LOT_SIZE=get_symbol_spec(symbol).volume_min,
            MAGIC=MAGIC,
        ...
        if symbol in active_trades_virtual:
            ...
            if existing['side'] != side and AUTO_CLOSE_ON_STRONG and confidence > 0.85:
        ...
        if symbol not in active_trades_virtual:
            ...
            if DRY_RUN:
                ...
                if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                    ...
                    _pending_signals[res.order] = sig
